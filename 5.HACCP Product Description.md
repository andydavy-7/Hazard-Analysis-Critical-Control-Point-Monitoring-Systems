...
│  Create, maintain, and version HACCP plans with AI         │
│                                                             │
│  MODULE 2: Daily Monitoring & Data Capture                 │
│  Mobile-first data collection with offline capability      │
│                                                             │
│  MODULE 3: Corrective Action Management                    │
│  Guided workflows for violations and incidents             │
│                                                             │
│  MODULE 4: Audit & Reporting                               │
│  One-click audit report generation and compliance tracking │
│                                                             │
│  MODULE 5: Multi-Location Management                       │
│  Centralized visibility across facilities and franchises   │
│                                                             │
│  MODULE 6: Integration & IoT                               │
│  Connect sensors, ERPs, and third-party systems            │
│                                                             │
│  MODULE 7: Administration & Settings                       │
│  User management, roles, permissions, and configuration    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                             │
│  ┌─────────────────────┐    ┌─────────────────────┐       │
│  │ HACCP Plan Service  │    │ Monitoring Service  │       │
│  │ • Plan generation   │    │ • Data capture      │       │
│  │ • AI assistance     │    │ • Validation        │       │
│  │ • Versioning        │    │ • Alerting          │       │
│  └─────────────────────┘    └─────────────────────┘       │
│                                                             │
│  ┌─────────────────────┐    ┌─────────────────────┐       │
│  │ Corrective Action   │    │ Reporting Service   │       │
│  │ • Workflow engine   │    │ • PDF generation    │       │
│  │ • Notifications     │    │ • Analytics         │       │
│  │ • Approvals         │    │ • Export            │       │
│  └─────────────────────┘    └─────────────────────┘       │
│                                                             │
│  ┌─────────────────────┐    ┌─────────────────────┐       │
│  │ Integration Service │    │ IoT Service         │       │
│  │ • ERP connectors    │    │ • Sensor data       │       │
│  │ • POS connectors    │    │ • Device mgmt       │       │
...
│  ┌─────────────────────┐    ┌─────────────────────┐       │
│  │ PostgreSQL          │    │ MongoDB             │       │
│  │ • Transactional data│    │ • Document storage  │       │
│  │ • Audit logs        │    │ • HACCP plans       │       │
│  │ • User data         │    │ • Templates         │       │
│  └─────────────────────┘    └─────────────────────┘       │
│                                                             │
│  ┌─────────────────────┐    ┌─────────────────────┐       │
...
┌─────────────────────────────────────────────────────────────┐
│ 1. USER TRIGGERS MONITORING TASK                           │
│    • Push notification or manual open                      │
│    • User opens mobile app                                 │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 2. LOCAL DATA CAPTURE (Offline-First)                      │
│    • App checks: Is data entry required?                   │
│    • User scans QR code (identifies CCP/location)          │
│    • User enters measurement (temp, pH, time)              │
│    • User takes photo evidence                             │
│    • App validates critical limits (local logic)           │
...
│    • Queue manager checks for unsynchronized records       │
│    • POST request to /api/v1/monitoring/sync               │
│    • Batch upload (multiple records at once)               │
│    • Server validates, persists to PostgreSQL              │
│    • Photos uploaded to S3                                 │
│    • Local records marked as synced                        │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 4. REAL-TIME ALERTING (Server-Side)                        │
│    • Server evaluates critical limits                      │
│    • If violation detected:                                │
│      - Create alert record in PostgreSQL                   │
│      - Publish to real-time notification service           │
│      - Send push notification to managers                  │
│      - Send SMS/email based on severity                    │
│      - Update dashboard (real-time via WebSocket)          │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 5. DASHBOARD UPDATE                                        │
│    • Manager dashboard auto-refreshes (WebSocket)          │
│    • Compliance metrics updated                            │
│    • Trend charts recalculated                             │
│    • Executive view updated (if multi-location)            │
└─────────────────────────────────────────────────────────────┘
```
...
| **Set Critical Limits** | AI suggests science-based critical limits | Reviews limits (e.g., "Internal temp ≥165°F for 15 sec"); adjusts if needed | Validates limits against FDA/USDA standards; flags if out of range |
...
```
┌─────────────────────────────────────────────────────────────┐
│ AI HACCP Plan Builder - Step 1 of 9                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
...
│  ○ Pharmaceuticals                                         │
│                                                             │
│  Great! What type of food service?                         │
│  [Dropdown: ▼ Quick Service Restaurant (QSR)           ]   │
│                                                             │
...
│  ┌─────────────────────────────────────────────┐          │
│  │ AI SUGGESTION:                               │          │
│  │ Based on QSR burgers, I recommend:          │          │
│  │ • CCP 1: Cooking (pathogen kill step)       │          │
│  │ • CCP 2: Cold holding (prevent growth)      │          │
│  │ • CCP 3: Produce washing (contamination)    │          │
│  └─────────────────────────────────────────────┘          │
│                                                             │
...
```
┌─────────────────────────────────────────────────────────────┐
│ AI HACCP Plan Builder - Step 4: Critical Control Points    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Your Process Flow:                                        │
│  ┌────────────────────────────────────────────┐           │
│  │ Receive Frozen Patties → Store in Freezer │           │
│  │         ↓                                  │           │
│  │ Cook on Grill → [CCP 1: COOKING]          │ ← CRITICAL│
│  │         ↓                                  │           │
│  │ Assemble Burger → Hold Hot                │           │
│  │         ↓                                  │           │
│  │ Serve to Customer                          │           │
│  └────────────────────────────────────────────┘           │
│                                                             │
│  🤖 AI Analysis:                                           │
│  CCP 1: COOKING is critical because:                      │
│  • Biological hazard: E. coli, Salmonella in raw beef     │
│  • Decision tree: Is control at this step necessary for   │
│    safety? YES                                             │
│  • Recommended critical limit: Internal temp ≥ 155°F      │
│    (FDA Food Code for ground beef)                         │
│                                                             │
│  ✏️ Your Adjustments:                                      │
│  Critical Limit: [155°F internal temperature for 15 sec]  │
│  Monitoring: [Every burger, digital thermometer]          │
│  Frequency: [Continuous during service]                   │
│  Responsible: [Line Cook on duty]                         │
│                                                             │
│  ✅ CCP 1 looks good!                                      │
│                                                             │
│  [◀ Back]  [Save & Continue to CCP 2 ▶]  [Skip This CCP]  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
    ...
    "hazard_analysis": [
      {
        ...
        "risk_level": "calculated (likelihood × severity)",
        ...
        "critical_limits": [
          {
            ...
            "limit_value": "string (e.g., '≥155°F')",
            "unit": "string (e.g., '°F')",
            "tolerance": "string (e.g., '+/- 2°F')"
          }
        ],
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Find a template for your industry:                        │
│  [🔍 Search templates...]                [Filter by: ▼]    │
│                                                             │
│  🍞 Food Manufacturing (23 templates)                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                │
│  │🥖 Bakery │  │🥛 Dairy  │  │🍖 Meat   │                │
│  │Products  │  │Processing│  │Processing│                │
│  │⭐⭐⭐⭐⭐   │  │⭐⭐⭐⭐    │  │⭐⭐⭐⭐⭐   │                │
│  │15 plans  │  │8 plans   │  │12 plans  │                │
│  └──────────┘  └──────────┘  └──────────┘                │
│                                                             │
│  🍔 Hospitality / Restaurant (18 templates)                │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                │
│  │🍔 QSR    │  │🍽️ Full   │  │🏨 Hotel  │                │
│  │Burgers   │  │Service   │  │Catering  │                │
│  │⭐⭐⭐⭐⭐   │  │⭐⭐⭐⭐    │  │⭐⭐⭐     │                │
│  │20 plans  │  │10 plans  │  │5 plans   │                │
│  └──────────┘  └──────────┘  └──────────┘                │
│                                                             │
│  💊 Pharmaceuticals (9 templates)                          │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                │
│  │💊 CMO    │  │🌿 Supple │  │⚗️ Compnd │                │
│  │Tablets   │  │-ments    │  │Pharmacy  │                │
│  │⭐⭐⭐⭐    │  │⭐⭐⭐⭐⭐   │  │⭐⭐⭐     │                │
│  │6 plans   │  │8 plans   │  │3 plans   │                │
│  └──────────┘  └──────────┘  └──────────┘                │
│                                                             │
│  [Load More Templates...]                                  │
│                                                             │
...
| **Approve/Reject Change** | QA manager reviews proposed plan amendment | Reviews changes in approval workflow; adds comments; approves or rejects | If approved: Increments version (e.g., 1.0 → 1.1); archives old version; If rejected: Notifies requestor with feedback |
...
```
┌─────────────────────────────────────────────────────────────┐
│ HACCP Plan: QSR Burger Operations - Version History        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Current Version: 2.1 (Active)                             │
...
│                                                             │
│  ┌──────────────────────────────────────────────┐         │
│  │ v2.1 (Active) - Dec 10, 2024                │         │
│  │ Modified by: Sarah Chen (QA Manager)        │         │
│  │ Change Summary: Updated cooking temp from   │         │
│  │ 155°F to 160°F per new FDA guidance        │         │
│  │ Status: ✅ Approved                         │         │
│  │ [View Details] [Compare with v2.0]         │         │
│  └──────────────────────────────────────────────┘         │
│                                                             │
│  ┌──────────────────────────────────────────────┐         │
│  │ v2.0 - Jun 5, 2024                          │         │
│  │ Modified by: Tom Martinez (Owner)           │         │
│  │ Change Summary: Added new CCP for produce   │         │
│  │ washing (E. coli outbreak prompted change)  │         │
│  │ Status: ✅ Approved                         │         │
│  │ [View Details] [Restore This Version]      │         │
│  └──────────────────────────────────────────────┘         │
│                                                             │
│  [Request Change to Current Plan]  [Schedule Review]       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ HACCP Plan: Panadería (Bakery) Operations                  │
│ Language: Español (Spanish) ▼                              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  CCP 1: HORNEADO (Cocción)                                │
│  Peligro Controlado: Patógenos (E. coli, Salmonella)      │
│                                                             │
│  Límite Crítico:                                           │
│  Temperatura interna ≥ 75°C (165°F) por 15 segundos       │
│                                                             │
│  Monitoreo:                                                │
│  ¿Qué?: Temperatura interna del pan                       │
│  ¿Cómo?: Termómetro digital de sonda                      │
│  ¿Cuándo?: Cada hornada                                   │
│  ¿Quién?: Panadero de turno                               │
│                                                             │
│  Acción Correctiva:                                        │
│  Si la temperatura es < 75°C:                              │
│  1. Continuar horneando hasta alcanzar 75°C               │
│  2. Verificar calibración del termómetro                   │
│  3. Registrar la acción tomada                             │
│  4. Notificar al supervisor                                │
│                                                             │
│  [⬅️ Anterior] [English | Español | 中文 | Tiếng Việt] [Siguiente ➡️] │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
    ...
    "terminology_glossary": [
      {
        ...
        "term_translated": "string (e.g., 'Punto Crítico de Control')",
        "definition": "text"
      }
    ]
  }
...
```
haccp_plan (1) ──< (M) plan_version
haccp_plan (1) ──< (M) plan_translation
haccp_template (1) ──< (M) haccp_plan (via template_id)
account (1) ──< (M) haccp_plan
facility (1) ──< (M) haccp_plan
user (1) ──< (M) haccp_plan (created_by, approved_by)
```
...
| **Enter Measurement** | User manually enters monitored value | Types temperature (e.g., "38°F") into number field | App validates in real-time; shows green (OK) or red (violation) indicator |
...
```
┌─────────────────────────────────────────────────────────────┐
│ 🔔 [9:00 AM] Time to check Freezer #3 Temperature         │
└─────────────────────────────────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  CCP 2: Cold Holding - Freezer #3                         │
│  Critical Limit: ≤ 0°F (-18°C)                             │
│                                                             │
│  ┌─────────────────────────────────────────┐              │
│  │ 📷 Scan QR code on freezer door        │              │
│  │                                         │              │
│  │     ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓                   │              │
│  │     ▓▓ QR CODE ▓▓                       │              │
│  │     ▓▓  SCAN   ▓▓                       │              │
...
│  │  -5  °F                     🎤            │            │
│  └───────────────────────────────────────────┘            │
│                     ⬆️ Tap mic for voice entry             │
│                                                             │
│  ✅ Within Limit (≤ 0°F)                                  │
│                                                             │
│  📸 Photo Evidence:                                        │
│  [📷 Take Photo of Thermometer Display]                   │
│                                                             │
│  ─────────────────────────────────────────                │
│                                                             │
│  Notes (optional):                                         │
...
│  Your Reading: 15°F                                        │
│                                                             │
│  ⚠️ Temperature is ABOVE safe limit!                       │
│                                                             │
...
│                                                             │
│  What did you do?                                          │
│  ☑ Closed freezer door immediately                        │
│  ☑ Checked product temp (still safe at 10°F)              │
│  ☐ Moved product to backup freezer                        │
│  ☐ Discarded affected product                             │
│                                                             │
│  📸 Take photo of thermometer:                             │
│  [Photo: thermometer_15F_1208_0902.jpg]                   │
│                                                             │
│  📸 Take photo of product (if affected):                   │
│  [📷 Take Photo]                                           │
│                                                             │
│  ✍️ Additional notes:                                      │
│  [Door was accidentally left open during stocking.         │
│   Closed immediately, will retrain staff.]                 │
│                                                             │
│  [❌ Cancel]                    [✅ Submit & Notify Manager]│
│                                                             │
│  ⚠️ Manager will be notified for review                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ Equipment Setup - Generate QR Codes                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Equipment List:                                           │
│                                                             │
│  ┌──────────────────────────────────────────────┐         │
│  │ FREEZER-003 (Walk-in Freezer #3)            │         │
│  │ Location: Production Floor, North Wall      │         │
│  │ CCP: CCP 2 - Cold Holding                   │         │
│  │ Critical Limit: ≤ 0°F                       │         │
│  │                                              │         │
│  │  ▓▓▓▓▓▓▓▓                                   │         │
│  │  ▓▓ QR  ▓▓  ← FREEZER-003                   │         │
│  │  ▓▓CODE ▓▓     Scan to Monitor               │         │
│  │  ▓▓▓▓▓▓▓▓                                   │         │
│  │                                              │         │
│  │  [📄 Print Label]  [📋 Copy QR Data]        │         │
│  └──────────────────────────────────────────────┘         │
│                                                             │
│  ┌──────────────────────────────────────────────┐         │
│  │ OVEN-001 (Commercial Oven #1)               │         │
│  │ Location: Kitchen, Station 2                │         │
│  │ CCP: CCP 1 - Cooking (Kill Step)            │         │
│  │ Critical Limit: ≥ 155°F internal             │         │
│  │                                              │         │
│  │  ▓▓▓▓▓▓▓▓                                   │         │
│  │  ▓▓ QR  ▓▓  ← OVEN-001                      │         │
│  │  ▓▓CODE ▓▓     Scan to Monitor               │         │
...
│  └──────────────────────────────────────────────┘         │
│                                                             │
│  [🖨️ Print All QR Codes (Label Sheet)]                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ ≡ HACCP Monitor                Settings → Sync Status     │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  📡 Connection Status: ⚠️ Offline (No WiFi)                │
│                                                             │
│  ┌─────────────────────────────────────────────┐          │
│  │ Pending Sync: 5 records                    │          │
│  │                                             │          │
│  │ • Freezer temp check (9:00 AM)             │          │
│  │ • Oven temp check (9:15 AM)                │          │
│  │ • Cooler temp check (9:30 AM)              │          │
│  │ • Freezer temp check (10:00 AM)            │          │
│  │ • Corrective action (10:15 AM)             │          │
│  │                                             │          │
│  │ Last successful sync: 8:45 AM               │          │
│  └─────────────────────────────────────────────┘          │
│                                                             │
│  ℹ️ Your data is saved locally. It will automatically      │
│     sync when you have internet connection.                │
│                                                             │
│  [🔄 Retry Sync Now] (grayed out - offline)               │
│                                                             │
│  ─────────────────────────────────────────                │
│                                                             │
│  Local Storage:                                            │
...
│  Photos stored: 89                                         │
│                                                             │
│  [🗑️ Clear Synced Data (Free Up Space)]                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ 📡 WiFi Connected - Syncing...                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Uploading 5 records to server                             │
│                                                             │
│  ████████████████░░░░░░░░  60% (3 of 5)                    │
│                                                             │
│  ✅ Freezer temp check (9:00 AM) - Synced                  │
│  ✅ Oven temp check (9:15 AM) - Synced                     │
│  ✅ Cooler temp check (9:30 AM) - Synced                   │
│  ⏳ Freezer temp check (10:00 AM) - Uploading...           │
│  ⏹️ Corrective action (10:15 AM) - Waiting...              │
│                                                             │
│  Uploading photos: 2 of 3 complete                         │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                             │
│  CCP 2: Cold Holding - Freezer #3                         │
│  Temperature: 15°F (VIOLATION)                             │
│                                                             │
│  📸 Photo Evidence (Required for violations):              │
│                                                             │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                │
│  │          │  │          │  │   ➕     │                │
│  │  Photo 1 │  │  Photo 2 │  │   Add    │                │
│  │ Thermo-  │  │ Product  │  │  Photo   │                │
│  │  meter   │  │  In      │  │          │                │
│  │ Reading  │  │ Freezer  │  │          │                │
│  │          │  │          │  │          │                │
│  │ [✏️ Edit] │  │ [✏️ Edit] │  │          │                │
...
│  └──────────┘  └──────────┘  └──────────┘                │
│                                                             │
│  Photo 1: thermometer_15F_120824.jpg                       │
│  Taken: Dec 8, 2024 10:15 AM                               │
│  By: Maria Rodriguez                                       │
│  Size: 0.8 MB                                              │
│                                                             │
│  [📷 Take Photo]  [🖼️ Choose from Gallery]  [🎥 Record Video] │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
**Entity Relationships:**
```
haccp_plan (1) ──< (M) monitoring_record
critical_control_point (1) ──< (M) monitoring_record
equipment (1) ──< (M) monitoring_record
user (1) ──< (M) monitoring_record
monitoring_record (1) ──< (M) media_evidence
monitoring_record (1) ──> (1) corrective_action (if violation)
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ 🚨 Corrective Action Wizard - Step 1 of 5                  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Critical Limit Violation Detected                         │
│                                                             │
│  CCP 2: Cold Holding - Freezer #3                         │
│  Critical Limit: ≤ 0°F                                     │
│  Your Reading: 15°F (OVER by 15°F)                        │
│  Time: Dec 8, 2024 10:15 AM                                │
│                                                             │
│  ❓ What caused this violation?                            │
│  (Select the most likely reason)                           │
│                                                             │
│  ○ Equipment malfunction (compressor, thermostat)          │
│  ● Door left open/not sealed properly                      │
│  ○ Power outage or electrical issue                        │
│  ○ Overloading (too much warm product added)               │
│  ○ Unknown - needs investigation                           │
│  ○ Other: [___________________________]                    │
│                                                             │
│  [Cancel]                          [Next: Actions Taken ▶] │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ 🚨 Corrective Action Wizard - Step 2 of 5                  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Root Cause: Door left open                                │
│                                                             │
│  ✅ What actions did you take?                             │
│  (Check all that apply)                                    │
│                                                             │
│  ☑ Closed freezer door immediately                        │
│  ☑ Checked product temperature (safe if ≤ 41°F)           │
│  ☑ Verified door seal is intact                           │
│  ☐ Moved product to backup freezer                        │
│  ☐ Called maintenance to repair                           │
│  ☐ Discarded affected product                             │
│                                                             │
│  ➕ Add custom action: [_______________________]           │
│                                                             │
│  ─────────────────────────────────────────                │
│                                                             │
│  📝 Additional details (optional):                         │
│  [Door was left open during stocking. Closed              │
│   immediately. Product temp still at 10°F (safe).]        │
│                                                             │
│  [◀ Back]                      [Next: Product Disposition ▶] │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ 🚨 Corrective Action Wizard - Step 3 of 5                  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  🍖 What happened to the product?                          │
│                                                             │
│  ○ Discarded (thrown away)                                 │
│  ○ Reworked (re-cooled, reprocessed)                      │
│  ● Released (product is safe)                              │
│  ○ On Hold (awaiting QA decision)                         │
│                                                             │
│  ─────────────────────────────────────────                │
│                                                             │
│  ✅ Released - Product is Safe                             │
│                                                             │
│  Why is the product safe despite the violation?           │
│  [Product temp was only 10°F (well below 41°F danger      │
│   zone). Door was open <5 minutes. No safety risk.]       │
│                                                             │
...
│  [50 lbs]                                                  │
│                                                             │
│  ⚠️ Manager approval required for releasing product        │
│     after violation.                                       │
│                                                             │
│  [◀ Back]                          [Next: Photo Evidence ▶] │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ 🔔 Pending Approvals (3)                  👤 Sarah (Manager)│
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────────────────────────────────────────┐         │
│  │ 🚨 HIGH PRIORITY                             │         │
│  │ Corrective Action #CA-2024-1208-003         │         │
│  │                                              │         │
│  │ CCP 2: Cold Holding - Freezer #3            │         │
│  │ Violation: 15°F (Limit: ≤ 0°F)              │         │
│  │ Initiated: 10:15 AM by Maria Rodriguez      │         │
│  │ Status: ⏳ Pending Your Approval             │         │
│  │                                              │         │
│  │ Root Cause: Door left open during stocking  │         │
│  │ Actions: Closed door, checked product temp  │         │
│  │ Product: Released (50 lbs, safe at 10°F)    │         │
│  │                                              │         │
│  │ 📸 Evidence: 2 photos attached               │         │
│  │                                              │         │
│  │ [👁️ Review Full Details]                    │         │
│  └──────────────────────────────────────────────┘         │
│                                                             │
│  ┌──────────────────────────────────────────────┐         │
│  │ Corrective Action #CA-2024-1208-002         │         │
│  │ CCP 1: Cooking - Oven #1                    │         │
│  │ Initiated: 9:45 AM by John Smith            │         │
│  │ [👁️ Review]                                 │         │
│  └──────────────────────────────────────────────┘         │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
```
┌─────────────────────────────────────────────────────────────┐
│ Corrective Action Review - CA-2024-1208-003                │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  📋 VIOLATION DETAILS                                      │
│  CCP 2: Cold Holding - Freezer #3                         │
│  Critical Limit: ≤ 0°F (-18°C)                             │
│  Actual Reading: 15°F                                      │
│  Deviation: +15°F OVER LIMIT                               │
│  Detected: Dec 8, 2024 10:15 AM                            │
│  By: Maria Rodriguez (Floor Staff)                         │
│                                                             │
│  📝 ROOT CAUSE                                             │
│  Primary: Door left open/not sealed                        │
│  Details: Door was accidentally left open during           │
│  stocking. Closed immediately. Duration <5 minutes.        │
│                                                             │
│  ✅ ACTIONS TAKEN                                          │
│  • Closed freezer door immediately (10:15 AM)              │
│  • Checked product temp: 10°F (safe, <41°F limit)         │
│  • Verified door seal is intact (no damage)                │
│                                                             │
│  🍖 PRODUCT DISPOSITION                                    │
│  Decision: ✅ Released (Product is Safe)                   │
│  Justification: Product temp 10°F (well below danger       │
│  zone). Door open <5 min. No safety risk.                 │
│  Batch: LOT-2024-1208-A                                    │
│  Quantity: 50 lbs                                          │
│                                                             │
│  📸 EVIDENCE (2 photos)                                    │
│  [Photo 1: Thermometer showing 15°F]                      │
│  [Photo 2: Product temp probe showing 10°F]               │
│                                                             │
│  ─────────────────────────────────────────                │
│                                                             │
│  💬 Manager Comments (optional):                           │
│  [Approved. Good catch. Remind staff to double-check       │
│   door closure after stocking.]                            │
│                                                             │
│  [❌ Reject (Request Changes)]  [✅ Approve Corrective Action]│
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| View Corrective Action Trends | QA manager reviews patterns over time | Opens "Corrective Actions" → "Trends" | Shows chart: violations by CCP, category, root cause over 30/60/90 days |
| Identify Recurring Issue | System detects pattern (same equipment failing repeatedly) | (Automatic) 3+ violations on same equipment in 30 days | Alert: "🚨 Recurring Issue Detected: Walk-in Freezer #2 (4 violations in 30 days)" |
| Run Root Cause Analysis | Manager investigates why Freezer #2 keeps failing | Taps alert → "Analyze Root Cause" | AI analyzes 4 violations; suggests: "Pattern: Door seal failures. Possible causes: Worn seal, incorrect closing procedure, high foot traffic" |
| Assign Investigation | Manager assigns maintenance to investigate | Selects "Assign Investigation" → Maintenance Manager | Notification sent; investigation task created |
...
| Mark Issue Resolved | Seal replaced; violations stop | Marks work order complete; monitors for 30 days | System monitors; if no new violations, flags issue as "✅ Resolved" |
...
│                                                            │
│  📊 Violations by CCP (Last 30 Days)                      │
│  ┌────────────────────────────────────────────────────┐   │
│  │ CCP-2: Cooking Temp    ████████ 8 violations       │   │
│  │ CCP-1: Receiving Temp  █████ 5 violations          │   │
│  │ CCP-3: Cooling         ███ 3 violations            │   │
│  │ CCP-4: Cold Storage    ██ 2 violations             │   │
│  └────────────────────────────────────────────────────┘   │
│                                                            │
│  🚨 RECURRING ISSUES DETECTED                              │
│                                                            │
│  ┌────────────────────────────────────────────────────┐   │
│  │ 🧊 Walk-in Freezer #2 (CCP-4)                      │   │
│  │ 4 violations in 30 days                             │   │
│  │ Pattern: Door left open / not sealed                │   │
│  │                                                      │   │
│  │ Root Cause (AI Analysis):                           │   │
│  │ • Door seal appears worn (3 of 4 violations)       │   │
│  │ • High foot traffic area (stocking 6x/day)         │   │
│  │ • Staff may not be fully closing door              │   │
│  │                                                      │   │
│  │ Recommended Actions:                                 │   │
│  │ ☑ Inspect/replace door seal                        │   │
│  │ ☑ Retrain staff on proper door closure             │   │
│  │ ☑ Install door alarm (alerts if open >2 min)      │   │
│  │                                                      │   │
│  │ [Assign Investigation] [Create Work Order]          │   │
│  └────────────────────────────────────────────────────┘   │
│                                                            │
│  ┌────────────────────────────────────────────────────┐   │
│  │ 🥩 Ground Beef Supplier (CCP-1)                    │   │
│  │ 3 violations in 60 days                             │   │
│  │ Pattern: Product arrives above 41°F                │   │
│  │                                                      │   │
│  │ Root Cause (AI Analysis):                           │   │
│  │ • Supplier's refrigerated truck may be faulty      │   │
│  │ • Long delivery times (4+ hours)                   │   │
│  │                                                      │   │
│  │ Recommended Actions:                                 │   │
│  │ ☑ Contact supplier; require temp logs              │   │
│  │ ☑ Audit supplier's cold chain                      │   │
│  │ ☑ Consider alternate supplier                      │   │
│  │                                                      │   │
│  │ [Contact Supplier] [Schedule Audit]                 │   │
│  └────────────────────────────────────────────────────┘   │
│                                                            │
└────────────────────────────────────────────────────────────┘
```
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| Monitor for Early Warning Signals | System tracks measurements trending toward limits | (Automatic) Daily analysis | Analyzes 7-day trend: "Walk-in cooler temps averaging 38°F (limit: 41°F); creeping upward" |
| Generate Preventive Alert | System detects potential future violation | (Automatic) Trend detected | Alert: "⚠️ Preventive Alert: Walk-in Cooler CCP-4 trending toward limit" |
...
| Reject Recommendation | Manager disagrees (false positive) | Taps "Dismiss" → Reason: "Equipment recently serviced; trend is normal seasonal variation" | Alert dismissed; AI learns from feedback |
| Track Preventive Action | Maintenance performs action | Updates work order: "Cleaned condenser coils; temps now stable at 35°F" | Action logged; system monitors for 14 days to confirm issue resolved |

**User Interface:**

```
┌──────────────────── PREVENTIVE ACTION DASHBOARD ─────────┐
│                                                            │
│  ⚠️ EARLY WARNING SIGNALS (3 Active)                      │
│                                                            │
│  ┌────────────────────────────────────────────────────┐   │
│  │ 🧊 Walk-in Cooler CCP-4 (High Priority)            │   │
│  │                                                      │   │
│  │ 📈 Trend Analysis:                                  │   │
│  │ Temperature creeping upward over 7 days              │   │
│  │ Dec 1: 35°F → Dec 8: 38°F (limit: 41°F)            │   │
│  │                                                      │   │
│  │ 🔮 Prediction:                                       │   │
│  │ Likely to exceed limit by: Dec 22 (14 days)         │   │
│  │ Confidence: 78%                                      │   │
│  │                                                      │   │
│  │ 💡 Recommended Preventive Actions:                   │   │
│  │ 1. Clean condenser coils (likely cause)            │   │
│  │ 2. Check refrigerant levels                         │   │
│  │ 3. Inspect door seals                               │   │
│  │                                                      │   │
│  │ [Create Work Order] [Dismiss Alert]                 │   │
│  └────────────────────────────────────────────────────┘   │
│                                                            │
│  ┌────────────────────────────────────────────────────┐   │
│  │ 👤 Staff Compliance: John Doe (Medium Priority)    │   │
│  │                                                      │   │
│  │ 📊 Compliance Trend:                                │   │
│  │ On-time monitoring completion declining              │   │
│  │ Nov: 98% → Dec: 89% (target: 95%)                  │   │
│  │                                                      │   │
│  │ 💡 Recommended Actions:                              │   │
│  │ 1. One-on-one coaching session                      │   │
│  │ 2. Review task schedule (possible overload?)       │   │
│  │                                                      │   │
│  │ [Schedule Coaching] [Review Schedule] [Dismiss]      │   │
│  └────────────────────────────────────────────────────┘   │
│                                                            │
│  ┌────────────────────────────────────────────────────┐   │
│  │ 🔧 Oven #3 CCP-2 (Low Priority)                    │   │
│  │                                                      │   │
│  │ 📈 Trend: Cooking times increasing slightly        │   │
│  │ Nov avg: 12 min → Dec avg: 13.5 min                │   │
│  │ (May indicate heating element wearing out)          │   │
│  │                                                      │   │
...
- `corrective_action` → `monitoring_record` (1:1, which monitoring event triggered this?)
- `corrective_action` → `approval_log` (1:N, multi-tier approvals)
- `root_cause_analysis` → `corrective_action` (1:N, analyzes multiple violations)
- `preventive_action` → `work_order` (1:1, if recommendation accepted)
...
- vs. Paper-based: "2-week audit prep → 30 seconds"
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| Select Audit Type | QA manager chooses regulatory framework | Opens "Audit Reports" → Selects "FDA Retail Audit" | System loads FDA-specific report template |
...
| Generate Report | System compiles all documentation | Taps "Generate Report" | Progress bar: "Compiling monitoring records... (3 sec) → Analyzing corrective actions... (2 sec) → Generating PDF... (1 sec)" |
...
| Share with Auditor | Grant read-only access to auditor | Taps "Share" → Enters auditor email: auditor@agency.gov | Auditor receives secure link; can browse records online (no download/edit rights) |
...
│  📋 Report Type:                                          │
│  ┌────────────────────────────────────────────────────┐   │
│  │ ● FDA Retail Food Audit (2017 Food Code)          │   │
...
│                                                            │
│  🎯 Scope:                                                │
│  ☑ All Critical Control Points (7 CCPs)                  │
│  ☑ All Monitoring Records (2,847 records)                │
│  ☑ All Corrective Actions (23 violations)                │
│  ☑ Training Records (12 employees)                       │
│  ☑ Supplier Documentation (8 suppliers)                  │
│  ☑ Equipment Calibration Logs (15 thermometers)          │
│                                                            │
│  📄 Report Options:                                       │
│  ☑ Include photos (evidence from corrective actions)     │
│  ☑ Include trend charts (compliance over time)           │
│  ☑ Include employee signatures (digital signatures)      │
│  ☐ Anonymize employee names (use IDs only)               │
│                                                            │
│  ───────────────────────────────────────────              │
│                                                            │
│  [Cancel]              [Generate Report (Estimated: 30s)] │
│                                                            │
└────────────────────────────────────────────────────────────┘
...
│                                                            │
│  ✅ Loading HACCP Plan (CCP definitions, critical limits) │
│  ✅ Compiling monitoring records (2,847 records)          │
│  ✅ Analyzing corrective actions (23 violations)          │
│  🔄 Generating trend charts...                            │
│  ⏸ Formatting PDF (87 pages)...                          │
│                                                            │
│  ████████████████░░░░ 80%                                 │
│                                                            │
...
│                                                            │
│  📄 FDA Retail Food Audit Report                          │
│  Facility: Artisan Bakery & Cafe                          │
│  Period: Jan 1 - Dec 31, 2024                             │
│  Pages: 87                                                 │
│  Generated: Dec 8, 2024 3:42 PM                           │
│                                                            │
│  📊 Summary:                                              │
│  • 2,847 monitoring records (100% compliance)             │
│  • 23 corrective actions (all resolved within 24 hours)  │
│  • 0 critical violations                                  │
│  • 12 employees trained (100% up-to-date)                │
│  • 8 suppliers (all approved, documentation current)      │
│                                                            │
│  [📱 Preview PDF]  [⬇️ Download]  [🖨 Print]  [📧 Share] │
│                                                            │
└────────────────────────────────────────────────────────────┘
```
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| View Dashboard | Manager checks compliance status | Opens app → "Dashboard" | Displays compliance score (98%), violations (2), overdue tasks (0) |
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| View Trends | Review historical performance | Opens "Analytics" → "Trends" | Shows 12-month compliance chart |
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| Create Report | Build custom report | Opens "Reports" → "Create Custom" | Report builder loads |
...
| Schedule Recurring | Automate report generation | Sets "Monthly on 1st" → Recipients | Scheduled; auto-emails monthly |
...
| Send Alert | Notify location manager | Taps "Send Alert" → Message | Push notification sent to manager |
...
| Track Resolution | Monitor corrective actions | Views incident dashboard | Shows timeline: violation detected → escalated → resolved |
...
| Receive Violation Alert | Sensor detects out-of-range temp | (Automatic) Temp exceeds 41°F | Alert sent to manager + floor staff immediately |
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| Generate API Key | Create credentials for integration | Opens "API" → "Generate Key" | API key created; shows documentation link |
...
| Deactivate User | Remove access | Taps "Deactivate" → Confirms | User account disabled; no longer can log in |
...
| Operation | Description | User Action | System Response |
|-----------|-------------|-------------|-----------------|
| Configure Violation Alerts | Set who gets notified | Selects: "Critical violations → SMS to QA Manager + Floor Staff" | Config saved |
...
| Choose Units | Temp scale preference | Selects "Fahrenheit" | All temps displayed as °F |
...
facility (1) ────── (N) haccp_plan
haccp_plan (1) ────── (N) critical_control_point
critical_control_point (1) ────── (N) monitoring_record
monitoring_record (1) ────── (1) corrective_action
corrective_action (1) ────── (N) approval_log
facility (1) ────── (N) user
facility (1) ────── (N) iot_sensor
facility (1) ────── (N) equipment
facility (1) ────── (1) erp_integration
```
...
- User activation rate: 85%+ (sign-up → active usage within 7 days)
...
- Time saved per week: 10 hours → 30 minutes (95% reduction)
